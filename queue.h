...
void q_free(queue_t *q) { dll_free(q); }

int q_isempty(const queue_t *q) { return dll_isempty(q); }

size_t q_size(const queue_t *q) { return dll_size(q); }

int enqueue(queue_t *q, const void *elem) {
    return dll_inserttail(q, elem) != NULL;
}

int dequeue(queue_t *q, void *out) { return dll_deletehead(q, out); }

int peek(const queue_t *q, void *out) { return dll_gethead(q, out); }
#endif // QUEUE_IMPLEMENTATION
#endif // QUEUE_H
