...
int ll_isempty(const linklist_t *l) { return !l || !l->head; }

size_t ll_size(const linklist_t *l) {
    size_t s = 0;
    for (const ll_node *cur = l->head; cur; cur = cur->next)
        s++;
    return s;
}
...
int ll_deletetail(linklist_t *l, void *out_elem) {
    ...
    if (l->head->next == NULL)
        return ll_deletehead(l, out_elem);
