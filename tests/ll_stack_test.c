...
static void test_struct_stack(void) {
    ...
    check(stack_pop(&st, &out) == 1 && out.id == 2 && strcmp(out.name, "Bob") == 0, "pop struct");
    check(stack_pop(&st, &out) == 1 && out.id == 1 && strcmp(out.name, "Ada") == 0, "pop struct");
...
int main(void) {
    test_int_stack();
    test_struct_stack();
    puts("All ll_stack tests passed.");
    return 0;
}
