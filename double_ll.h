...
int dll_isempty(const double_ll_t *l) { return !l || !l->head; }

size_t dll_size(const double_ll_t *l) {
    size_t s = 0;
    for (const dll_node *cur = l->head; cur; cur = cur->next)
        s++;
    return s;
}
...
int dll_delete(double_ll_t *l, size_t i, void *out_elem) {
    if (dll_isempty(l) || !out_elem)
        return 0;
    if (i == 0)
        return dll_deletehead(l, out_elem);
